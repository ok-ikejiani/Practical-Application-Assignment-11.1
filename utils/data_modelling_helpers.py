...
def train_baseline_model(X_train, y_train, model_type='classification', scoring=None, cv=5):
    """
...
def train_models(models_list, pipeline, X_train, y_train, model_type='classification', scoring =None, cv=5, verbose=0):
    """
    Trains multiple models using GridSearchCV and returns the best model, parameters, score, and training time for each model based on the scoring metric.
    """
...
def plot_model_performance(results_df, model_type, graph_title=None):
    ...
    if model_type == 'classification':
        ...
        create_performance_bar_chart(axs[1, 1], f1_values, 'F1 Score', models)  
    else:
